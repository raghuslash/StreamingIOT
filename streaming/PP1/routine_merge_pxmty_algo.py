    ...
    while (END_FLAG == 0):
        ...
        if (i >= len(inp_df)):                     #already reached end of dataframe
            ...
            if (inp_df.weightage[i] <= THRESHOLD):
                str_tmstmp = str(pd.Timestamp(inp_df.arvl_tmstmp[i]) - pd.Timedelta('0 days 00:00:05.000000'))
                str_tmstmp = str_tmstmp.replace(' ','T')                
                end_tmstmp = str(pd.Timestamp(inp_df.arvl_tmstmp[i]) + pd.Timedelta('0 days 00:00:05.000000'))              
