...
# In[2]:
pp2_raw=pd.read_csv("PP2_Processed_DataFrame.csv", usecols=['timestamp','quant_sig'])
pp2_raw.dropna(axis=0, inplace=True)
pp2_raw.timestamp=pd.to_datetime(pp2_raw.timestamp)
# pp2_raw['acc']=(pp2_raw["data.ax"]**2+pp2_raw["data.az"]**2)**0.5
# pp2_raw.timestamp=pp2_raw.timestamp + datetime.timedelta(hours=5, minutes=30)


# In[3]:


pp2_raw.rename({"quant_sig":"acc"}, axis=1, inplace=True)


# In[4]:


pp2_processed=pd.read_csv("PP2_Processed_DataFrame.csv", usecols=['timestamp', 'mrg_binry_sig'])
pp2_processed.dropna(axis=0, inplace=True)
pp2_processed.timestamp=pd.to_datetime(pp2_processed.timestamp)
# pp2_processed.timestamp=pp2_processed.timestamp + datetime.timedelta(hours=5, minutes=30)


# In[5]:


pp2_raw.sort_values('timestamp', axis=0, inplace=True)
pp2_processed.sort_values('timestamp', axis=0, inplace=True)


# In[6]:


# print(pp2_raw.shape[0], pp2_processed.shape[0])
...
#plt.plot(pp2_raw.timestamp, pp2_raw.acc)
#plt.plot(pp2_processed.timestamp, pp2_processed.mrg_binry_sig)


# In[9]:


# pp2_raw=pp2_raw[(pp2_raw['timestamp'] >= pp2_processed['timestamp'][0]) & (pp2_raw['timestamp'] <= pp2_processed.iloc[-1].timestamp)]


# In[10]:


# pp2_raw.reset_index(inplace=True)
# pp2_raw.drop('index', axis=1, inplace=True)
...
downtimes=pp2_raw.copy()
downtimes['mrg_binry_sig']=pp2_processed['mrg_binry_sig']
...
# plt.plot(pp2_raw.timestamp, pp2_raw.acc+0.5)
...
if not os.path.exists('/mnt/UltraHD/streamingAno/PP2'):

    os.makedirs('/mnt/UltraHD/streamingAno/PP2')
...
directory='/mnt/UltraHD/streamingAno/PP2'


timestr = time.strftime("%Y-%m-%d_%H-%M-%S")
print("Saving Anomalous events with file name -", timestr+"_PP2.csv")
PP2_ano_events.to_csv('/mnt/UltraHD/streamingAno/PP2/'+ timestr+"_PP2.csv")
...
# plt.plot(pp2_raw.timestamp, pp2_raw.acc*40, 'orange')
